...
void _VFS::InitAndLoadInternalCatalog(unsigned search_for_embeddings)
{
    ...
    XLOG | _S* _XOr("<VFS> embedded root '%s'",25,466426447) % root_path_;
    ...
    if (!appfolder_handle_ || appfolder_handle_ == INVALID_HANDLE_VALUE)
        XLOG | _XOr("<VFS/ERR> failed to open appfolder",35,468589106);

    if (!catalog_ && search_for_embeddings)
        XLOG | _S* _XOr("<VFS> failed to open embedded packages catalog: %s",51,436607498) % +sefa;
}
...
void _VFS::_AfterMountPackage(svfs::PackagePtr& pkg, pchar_t pkgname, pchar_t intern, StringEfA& sefa)
{
    if (pkg)
    {
                ...
                if (pkg->StreamKind(i) == 1)
                {
                    ...
                    if (pkg->GetStreamData(i, register_data, sefa))
                    {
                        ...
                        if (!Import_Registry(+register_data, error))
                            xlog | _S* _XOr("failed to register regfile from stream %d: %s ",47,441392589) % i % error;
                    }
                }
        }
        else
        ...
        XLOG | _S* _XOr("<VFS> package '%s' has been mounted %s",39,444079512)
        % pkgname
        % intern;
    }
    else
        XLOG | _S* _XOr("<VFS> failed to mount %s package '%s': %s",42,447618418)
        % intern
        % pkgname
        % +sefa;
}
...
{
    byte_t sign[16];
    svfs::Md5S((pwd && pwd[0]) ? pwd : _XOr("password",9,450764098), sign);
...
{
    byte_t sign[16];
    svfs::Md5S((pwd && pwd[0]) ? pwd : _XOr("password",9,449846576), sign);
...
unsigned _VFS::MountS_At(DataStreamPtr ds, pchar_t pkgname, cbyte_t* pwdsign, i64_t offs)
{
    ...
    if (ds)
    {
        ...
        {
            Unrefe(pkg);
            XLOG | _S* _XOr("%s: %s",7,451091745) % pkgname % +sefa;
        }
    }
    else
        XLOG | _S* _XOr("%s: '%s does not exist'",24,452205844) % pkgname % ds->Identifier();

    if (pkg)
    {
        ++count;
        _AfterMountPackage(pkg, pkgname, _XOr("externally",11,488250622), sefa);
    }
...
unsigned _VFS::MountS(pwide_t mask, cbyte_t* pwdsign, bool atAppfold)
{
    ...
    XLOG | _S* _XOr("mounting packages by mask %s ...",33,489430252) % pkgmask;
    if (pkgmask)
    {
        if (atAppfold)
        {
    ...
    if (catalog_ && !defaults_mounted)
        for (int i = 1, iE = catalog_->MntsCount(); i <= iE ; ++i)
        {
            if (catalog_->IsIntrMount(i))
            {
                ...
                if (pkg)
                    _AfterMountPackage(pkg, +pkgname, _XOr("internally",11,491461833), sefa);
            }
            else
