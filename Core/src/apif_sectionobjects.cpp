        ...
        if ( HOA::R o = handles_.Get(args->SectionHandle) )
        {
                ...
                __lockon__(VFS)
            {
                *result = o->MapView(
                              ...
                              args->BaseAddress,
                              args->ViewSize);
                XLOG|_S* _XOr("<APIF/VFS> mmap section (%08x) '%s\\%s' at %08x - %08x -> %08x",62,
                              327054146) %args->SectionHandle %o->PackageName() %o->ComposePath() %*args->BaseAddress %(args->ViewSize?*args->ViewSize:
                                      o->Size())
                %*result;
