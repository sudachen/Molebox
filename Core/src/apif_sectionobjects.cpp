...
struct SectionObjectFilter : APIfilter
{
    ...
    APIF_STATUS DoCloseObject(void* _a, unsigned* result)
    {
        ...
        //XDBG|_S*_XOr(".apif.CloseObject %08x",23,269578289) % args->FileHandle;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            if (o)
            {
                __lockon__(VFS)
                if ( o->IsValid() )
                    XLOG|_S* _XOr("<APIF/VFS> close section (%08x) '%s\\%s'",40,276131885)
                    %args->FileHandle
                    %o->PackageName()
                    %o->ComposePath();
    ...
    APIF_STATUS DoCreateSection(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.CreateSection file %08x, size %08x, page %08x, type %08x",63,274428027)
        %args->FileHandle
                ...
                if ( p->IsValid() )
                {
                    ...
                    if ( so->Create() )
                        ...
                        *args->SectionHandle = 0,
                               *result = STATUS_INVALID_HANDLE;
                    XLOG|_S* _XOr("<APIF/VFS> create section for '%s\\%s', attr %08x, size %08x -> %08x,%08x",73,283603903)
                    %p->PackageName()
                    %p->ComposePath()
                    %args->SectionAttributes
                    %size
    ...
    APIF_STATUS DoMapSection(void* _a, unsigned* result)
    {
        ...
        //__asm int 3
        ;
        XDBG|_S* _XOr(".apif.MapSection %08x, base %08x, offset %08x, size %08x, protect %08x",71,288715757)
        %args->SectionHandle
        %*args->BaseAddress
        ...
        if ( HOA::R o = handles_.Get(args->SectionHandle) )
        {
                ...
                __lockon__(VFS)
            {
                *result = o->MapView(
                              ...
                              args->ViewSize);
                XLOG|_S* 
		   _XOr("<APIF/VFS> mmap section (%08x) '%s\\%s' at %08x - %08x -> %08x",62,293565271) 
    ...
    APIF_STATUS DoQuerySection(void* _a, unsigned* result)
    {
        ...
        //XDBG|_S*_XOr(".apif.MapSection %08x, base %08x, protect %08x",47,297366169) %;
        //__asm int 3
        ;
        if ( HOA::R o = handles_.Get(args->SectionHandle) )
        {
            XLOG|_S* _XOr("<APIF/VFS> query section (%08x) '%s\\%s', Ic = %d",49,301363886)
    ...
    APIF_STATUS DoUnmapSection(void* _a, unsigned* result)
    {
        ...
        {
            XLOG|_S* _XOr("<APIF/VFS> unmmap section '%s\\%s' at %08x",42,304902884)
