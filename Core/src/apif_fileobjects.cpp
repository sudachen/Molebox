...
struct FileObjectFilter : APIfilter
{
        ...
        if ( oa->RootDirectory )
            ...
            else
            {
                ...
                else
                {
                    *inBox = false;
                    //__asm int 3;
                    XLOG|_S* _XOr("oppss, unknown path to %08x/%s",31,69011455)
    ...
    APIF_STATUS DoCreateFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.CreateFile '%s'",22,70911966)
        ...
        if ( FioProxyPtr p = NormalizeOAnGetProxy(&oa,internal_path,&inBox) )
        {
            ...
            else
            {
                ...
                *result = 0;
                XLOG|_S* _XOr("<APIF/VFS> open('%s\\%s') -> %08x",33,71370691)
    ...
    APIF_STATUS DoQueFileAttr(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.QueAttrFile '%s'",23,75171737)
        %StringW(args->ObjectAttributes->ObjectName->Buffer,args->ObjectAttributes->ObjectName->Length/2);
        ...
        if ( FioProxyPtr p = NormalizeOAnGetProxy(&oa,internal_path,&inBox) )
        {
            ...
            *result = 0;
            XLOG|_S* _XOr("<APIF/VFS> query attr of '%s\\%s' -> %08x",41,75499392)
    ...
    APIF_STATUS DoQueFileAttrFull(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.QueAttrFileFull '%s'",27,78907220)
        %StringW(args->ObjectAttributes->ObjectName->Buffer,args->ObjectAttributes->ObjectName->Length/2);
        ...
        if ( FioProxyPtr o = NormalizeOAnGetProxy(&oa,internal_path,&inBox) )
        {
            ...
            *result = 0;
            ...
            XLOG|_S* _XOr("<APIF/VFS> query full attr of '%s\\%s' -> %08x",46,81200949)
    ...
    APIF_STATUS DoOpenFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.OpenFile '%s'",20,117769991)
        %StringW(args->ObjectAttributes->ObjectName->Buffer,args->ObjectAttributes->ObjectName->Length/2);
        ...
        if ( FioProxyPtr p = NormalizeOAnGetProxy(&oa,internal_path,&inBox) )
        {
            ...
            *args->FileHandle = handles_.Put(Refe(+p));
            *result = 0;
            XLOG|_S* _XOr("<APIF/VFS> open('%s\\%s') -> %08x",33,120194798)
            %p->PackageName() %internal_path %*args->FileHandle;
            return APIF_RETURN;
        }
        else
    ...
    APIF_STATUS DoCloseFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.CloseFile %08x",21,122095301) % args->FileHandle;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            if (o)
            {
                __lockon__(VFS)
                if ( o->IsValid() )
                    XLOG|_S* _XOr("<APIF/VFS> close(%08x) '%s\\%s'",31,124520108)
                    %args->FileHandle
                    %o->PackageName()
                    %o->ComposePath();
    ...
    APIF_STATUS DoReadFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.ReadFile %08x, count:%d",30,126355080) % args->FileHandle % args->Length;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            if (!o)
                *result = STATUS_INVALID_HANDLE;
                ...
                __lockon__(VFS)
            {
                ...
                if ( o->IsRoot() )
                {
                    ...
                    XDBG|_S* _XOr("<APIF/VFS> access to application folder is redirectd to handle %08x",68,128517739) % args->FileHandle;
                    return APIF_CONTINUE;
                }
                ...
                XLOG|_S* _XOr("<APIF/VFS> read(%08x,%d,%d) '%s\\%s' -> %d,%08x",47,132187683)
                %args->FileHandle
                %a
                %args->Length
                %o->PackageName()
    ...
    APIF_STATUS DoWriteFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.WriteFile %08x, count:%d",31,103680498) % args->FileHandle % args->Length;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
                ...
                __lockon__(VFS)
            {
                ...
                {
                    args->FileHandle = VFS->AppFolderHandle();
                    XDBG|_S* _XOr("<APIF/VFS> access to application folder is redirectd to handle %08x",68,105843149) % args->FileHandle;
                    return APIF_CONTINUE;
                }
                *result = 0;
            }
    ...
    APIF_STATUS DoQueFilInfo(void* _a, unsigned* result)
    {
        ...
        //  __asm int 3
        //  ;
        XDBG|_S* _XOr(".apif.QueFilInfo %08x, %08x",28,109316484) % args->FileHandle % args->FileInformationClass;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            if (!o) *result = STATUS_INVALID_HANDLE;
            else
                __lockon__(VFS)
            {
                ...
                {
                    args->FileHandle = VFS->AppFolderHandle();
                    XDBG|_S* _XOr("<APIF/VFS> access to application folder is redirectd to handle %08x",68,111413604) % args->FileHandle;
                    return APIF_CONTINUE;
                }
                *result = 0;
                ...
                __lockon__(VFS)
            {
                ...
                *result = 0;
                if ( args->FileInformationClass == FilePositionInformation )
                {
                    ...
                    XLOG|_S* _XOr("<APIF/VFS> seek(%08x,%d(%08x)) '%s\\%s'",39,117246239)
                    %args->FileHandle
                    %o.State()
                    %o.State()
                    %o->PackageName()
    ...
    APIF_STATUS DoQueVolInfo(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.QueVolInfo %08x, %08x",28,152111347) % args->FileHandle % args->FileSystemInformationClass;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            if (!o) *result = STATUS_INVALID_HANDLE;
            else
                __lockon__(VFS)
            {
                ...
                {
                    args->FileHandle = VFS->AppFolderHandle();
                    XDBG|_S* _XOr("<APIF/VFS> access to application folder is redirectd to handle %08x",68,156174541) % args->FileHandle;
                    return APIF_CONTINUE;
                }
                else
                {
    ...
    APIF_STATUS DoQueDirFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.QueDirInfo %08x, %08x",28,159713415) % args->FileHandle % args->FileInformationClass;
                ...
                *result = 0;
                     ...
                     || args->FileInformationClass == FileOleDirectoryInformation )
                {
                    ...
                    if ( args->RestartScan || !ls )
                    {
                        ...
                        XLOG|_S* _XOr("<APIF/VFS> list('<workpath>\\%s|%s')  -> %d",43,161613926)
                        %o->ComposePath1()
                        %mask
                        %ls->Count();
    ...
    APIF_STATUS DoLockFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.DoLockFile %08x",22,164759606) % args->FileHandle;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            __lockon__(VFS)
            {
    ...
    APIF_STATUS DoUnlockFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.DoUnlockFile %08x",24,167249944) % args->FileHandle;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            __lockon__(VFS)
            {
    ...
    APIF_STATUS DoGetEnVar(void* args, unsigned* result)
    {
        ...
        if ( StrSafeEqualI(+name,_XOr(";molebox;;exefilter;",21,136252413)) )
        {
            *result = (unsigned)&CoreAPI_EnableExeFilter;
            return APIF_RETURN;
        }
        else if ( StrSafeEqualI(+name,_XOr(";molebox;;fsfilter;",20,136645607)) )
        {
            *result = (unsigned)&CoreAPI_EnableFsFilter;
            return APIF_RETURN;
        }
        else if ( StrSafeEqualI(+name,_XOr(";molebox;;mount;",17,139398093)) )
        {
            *result = (unsigned)&CoreAPI_Mount;
            return APIF_RETURN;
        }
        else if ( StrSafeEqualI(+name,_XOr(";molebox;;unmount;",19,139987896)) )
        {
            *result = (unsigned)&CoreAPI_Unmount;
            return APIF_RETURN;
        }
        else if ( StrSafeEqualI(+name,_XOr(";molebox;;coreapi;",19,140708769)) )
        {
    ...
    APIF_STATUS DoDuplicate(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.DuplicateFile %08x",25,143461263) %args->SourceHandle;
        if ( HOA::R o = handles_.Get(args->SourceHandle) )
        {
            __lockon__(VFS)
            {
                *args->TargetHandle = handles_.Put(Refe(+o));
                if ( o->IsValid() )
                    XLOG|_S* _XOr("<APIF/RGS> duplicate file (%08x) '%s' -> %08x",46,145623922)
                    %args->SourceHandle
                    %o->ComposePath()
                    %*args->TargetHandle;
                *result = 0;
    ...
    APIF_STATUS DoNotifChangeFile(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.NotifChangeFile %08x",27,148769602) %args->FileHandle;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            //__asm int 3;
            __lockon__(VFS)
            {
                ...
                if ( o->IsValid() )
                    XLOG|_S* _XOr("<APIF/RGS> query changes on (%08x) '%s\\%s' -> %08x",51,149097249)
                    %args->FileHandle
                    %o->PackageName()
                    %o->ComposePath()
                    %newH;
