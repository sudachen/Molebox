...
struct RegEntry : Refcounted
{
    ...
    RegEntry* Subkey(int i)
    {
        ...
        {
            XLOG|_S* _XOr("<ERR> invalid subkey index %d of key '%s' ['apif_register.cpp':%d]",67,152574728)
            %i
            %Path()
            %__LINE__;
            //__asm int 3;
    ...
    void Load(DataStreamPtr& ds)
    {
        ...
        while ( !ds->Error() )
        {
            ...
            if ( regS.Count() > 2 )
            {
                ...
                if ( logrgloading_ )
                    XDBG | _S* _XOr("RGLOAD: %s",11,181738317) %+regS;
...
struct RegObjectFilter : APIfilter
{
    ...
    APIF_STATUS DoCreateHkey(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.CreateHkey %08x/'%s'",27,183114618) %args->ObjectAttributes->RootDirectory %path;
        __lockon__(REGISTER)
        {
            RegEntryPtr par;
            ...
            if ( RegEntryPtr p = REGISTER->GetOrCreateEntryFiltered(+par,+path,&created) )
            {
                ...
                *result = 0;
                XLOG|_S* _XOr("<APIF/RGS> create hkey '%s' -> %08x, created: %?",49,176757401)
    ...
    APIF_STATUS DoCreateHkey_1(void* _a, unsigned* result)
    {
        NT_CREATE_HKEY_ARGS* args = (NT_CREATE_HKEY_ARGS*)_a;
        StringW path(args->ObjectAttributes->ObjectName->Buffer,args->ObjectAttributes->ObjectName->Length/2);
        XDBG|_S* _XOr(".apif.CreateHkey (unvirtualized) %08x/'%s'",43,179313360) %args->ObjectAttributes->RootDirectory %path;
        __lockon__(REGISTER)
        {
            RegEntryPtr par;
            if ( args->ObjectAttributes->RootDirectory )
    ...
    APIF_STATUS DoOpenHkey(void* _a, unsigned* result)
    {
        ...
        StringW path(args->ObjectAttributes->ObjectName->Buffer,args->ObjectAttributes->ObjectName->Length/2);
        XDBG|_S* _XOr(".apif.OpenHkey %08x/'%s'",25,174136033) %args->ObjectAttributes->RootDirectory %path;
        bool deleted = false;
        __lockon__(REGISTER)
        {
            RegEntryPtr par;
            ...
            if ( RegEntryPtr p = REGISTER->GetEntryFiltered(+par,+path,&deleted) )
            {
                *args->KeyHandle = handles_.Put(Refe(+p));
                *result = 0;
                XLOG|_S* _XOr("<APIF/RGS> open hkey '%s' -> %08x:%s",37,168040988)
            ...
            {
                *result = STATUS_OBJECT_NAME_NOT_FOUND;
                XLOG|_S* _XOr("<APIF/RGS> couldn't open hkey, the hkey is deleted '%s%s%s'",60,171907623)
            ...
            {
                XLOG|_S* _XOr("<APIF/RGS> open native hkey '%08x:%s'",38,200874601)
    ...
    APIF_STATUS DoOpenHkey_1(void* _a, unsigned* result)
    {
        NT_OPEN_HKEY_ARGS* args = (NT_OPEN_HKEY_ARGS*)_a;
        StringW path(args->ObjectAttributes->ObjectName->Buffer,args->ObjectAttributes->ObjectName->Length/2);
        XDBG|_S* _XOr(".apif.OpenHkey (unvirtualized) %08x/'%s'",41,195892661) %args->ObjectAttributes->RootDirectory %path;
        bool deleted = false;
        __lockon__(REGISTER)
        {
            RegEntryPtr par;
            if ( args->ObjectAttributes->RootDirectory )
                if ( !(par=QueryPar(args->ObjectAttributes->RootDirectory)) )
                    return APIF_CONTINUE;
            XLOG|_S* _XOr("<APIF/RGS> open native hkey '%08x:%s'",38,188749248)
            %args->ObjectAttributes->RootDirectory %path;
            if ( handles_._Get(args->ObjectAttributes->RootDirectory) )
                if ( HANDLE rootdir = _LookForRegKeyHandle(+par,0) )
                {
    ...
    APIF_STATUS DoDuplicateKey(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.DuplicateHkey %08x",25,192157164) %args->SourceHandle;
        if ( HOA::R o = handles_.Get(args->SourceHandle) )
        {
            ...
            if ( o->IsValid() )
            {
                *args->TargetHandle = handles_.Put(Refe(+o));
                XLOG|_S* _XOr("<APIF/RGS> duplicate hkey (%08x) '%s' -> %08x",46,186127624)
                %args->SourceHandle
                %o->Path()
                %*args->TargetHandle;
                *result = 0;
    ...
    APIF_STATUS DoCloseHkey(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.CloseHkey %08x",21,214570330) % args->FileHandle;
        if ( HOA::R o = handles_.Get(args->FileHandle) )
        {
            __lockon__(REGISTER)
            {
                if ( o->IsValid() )
                {
                    XLOG|_S* _XOr("<APIF/RGS> close hkey (%08x) '%s'",34,215029105)
                    %args->FileHandle
                    %o->Path();
    ...
    APIF_STATUS DoDeleteHkey(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.DeleteHkey %08x",22,210637974) % args->KeyHandle;
        if ( HOA::R o = handles_.Get(args->KeyHandle) )
        {
            ...
            __lockon__(REGISTER)
            {
                if ( o->IsValid() )
                {
                    XLOG|_S* _XOr("<APIF/RGS> delete hkey (%08x) '%s'",35,210900146)
                    %args->KeyHandle
                    %o->Path();
                    o->Delete();
                    _ClearRegKeyHandle(+o);
    ...
    void CloneNativeKey(HOA::R& o, HANDLE KeyHandle)
    {
        ...
        if ( HANDLE hk = _LookForRegKeyHandle(+o,KeyHandle))
            for (;; ++idx)
            {
                ...
                {
                    XLOG|_S* _XOr("<ERR> failed(%08x) to query key value %d of '%s'",49,206509271)
                    %st %idx %o->Path();
                    //__asm int 3;
                    continue;
                }
    ...
    APIF_STATUS DoDeleteHkeyVal(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.DeleteHkeyVal %08x %s",28,208934124) % args->KeyHandle % valname;
        if ( HOA::R o = handles_.Get(args->KeyHandle) )
        {
            __lockon__(REGISTER)
            {
                if ( o->IsValid() )
                {
                    XLOG|_S* _XOr("<APIF/RGS> delete hkey value (%08x) '%s'/'%s'",46,202773518)
                    %args->KeyHandle
                    %o->Path()
                    %valname;
                    if ( o->IsProxy() && REGISTER->IsFiltered() )
                        ...
                        {
                            XLOG|_S* _XOr("<APIF/RGS> delvalue '%s' redirecting to native registry",56,230822978) %valname;
                            args->KeyHandle = hk;
                            return APIF_CONTINUE;
                        }
    ...
    APIF_STATUS DoSetValHkey(void* _a, unsigned* result)
    {
        ...
        //__asm int 3;
        StringW valname(args->ValueName->Buffer,args->ValueName->Length/2);
        XDBG|_S* _XOr(".apif.SetValHkey %08x, val '%s', type %02x, size %d",52,226760604)
        % args->KeyHandle
        % valname
        % args->Type
        % args->DataSize;
            ...
            __lockon__(REGISTER)
            {
                if ( o->IsValid() )
                {
                    if ( o->IsProxy() && REGISTER->IsFiltered() )
                        if ( HANDLE hk = _LookForRegKeyHandle(+o,args->KeyHandle,1) )
                        {
                            XLOG|_S* _XOr("<APIF/RGS> setvalue '%s' redirecting to native registry",56,229840851) %valname;
                            args->KeyHandle = hk;
                            return APIF_CONTINUE;
                        }
                    XDBG|_S* _XOr("<APIF/RGS> proxy: %d, filtered: %d",35,225515281) %o->IsProxy() %REGISTER->IsFiltered();
                    valname.ToUpper();
                    XLOG|_S* _XOr("<APIF/RGS> set value '%s'=%s:'%s' of hkey(%08x) '%s'",53,221124406)
                    %valname
    ...
    APIF_STATUS DoQueryHkey(void* _a, unsigned* result)
    {
        ...
        //__asm int 3;
        XDBG|_S* _XOr(".apif.QueryHkey %08x",21,248911694) % args->KeyHandle;
        if ( HOA::R o = handles_.Get(args->KeyHandle) )
        {
            //if (!o)
            //  { *result = STATUS_INVALID_HANDLE; }
            ...
            __lockon__(REGISTER)
            {
                ...
                {
                    XLOG|_S* _XOr("<APIF/RGS> query hkey(%08x) '%s'",33,251467621)
                    %args->KeyHandle
                    %o->Path();
                    if ( o->IsProxy() )
                    {
    ...
    APIF_STATUS DoQueryValHkey(void* _a, unsigned* result)
    {
        ...
        //__asm int 3;
        XDBG|_S* _XOr(".apif.QueryValHkey %08x",24,244913801) % args->KeyHandle;
        if ( HOA::R o = handles_.Get(args->KeyHandle) )
        {
            __lockon__(REGISTER)
            {
                ...
                {
                    XLOG|_S* _XOr("<APIF/RGS> query value '%s' of hkey(%08x) '%s'",47,245569187)
    ...
    APIF_STATUS DoEnumHkey(void* _a, unsigned* result)
    {
        ...
        //__asm int 3;
        XDBG|_S* _XOr(".apif.EnumHkey %08x",20,242030325) % args->KeyHandle;
        if ( HOA::R o = handles_.Get(args->KeyHandle) )
        {
            //__asm int 3;
            if (o)
                __lockon__(REGISTER)
            {
                ...
                {
                    XLOG|_S* _XOr("<APIF/RGS> enum hkey(%08x) '%s'",32,242489068)
                    %args->KeyHandle
                    %o->Path();
    ...
    APIF_STATUS DoEnumValHkey(void* _a, unsigned* result)
    {
        ...
        //__asm int 3;
        XDBG|_S* _XOr(".apif.EnumValHkey %08x",23,236000817) % args->KeyHandle;
        if ( HOA::R o = handles_.Get(args->KeyHandle) )
        {
            //if (o)
            __lockon__(REGISTER)
            {
                ...
                {
                    XLOG|_S* _XOr("<APIF/RGS> enum values of hkey(%08x) '%s'",42,238687786)
                    %args->KeyHandle
                    %o->Path();
                    if ( !o->IsProxy() )
                    {
    ...
    HANDLE _LookForRegKeyHandle(RegEntry* e,HANDLE /*eh*/ = 0,unsigned /*rw*/ = 0)
    {
            ...
            //if ( long st = NtOpenKey(&minrkh->eh,KEY_READ|(rw?KEY_WRITE:0),&oa) )
        {
            ...
            {
                XLOG|_S* _XOr("<ERR> failed to open key '%s' with status %08x ['apif_register.cpp':%d]",72,266999418) 
			%rpath %st %__LINE__;
                //__asm int 3;
            }
            minrkh->eh = 0;
        ...
        {
            XDBG|_S* _XOr("<APIF/RGS> open shadow key %08x '%s'",37,264114598) %minrkh->eh %rpath;
            minrkh->stamp = ++stamp;
            minrkh->e = e;
        }
        return minrkh->eh;
    ...
    virtual APIF_STATUS DoQuerySecurity(void* _a,unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.QueryKeySecurity %08x",28,257298894) % args->ObjectHandle;
            ...
            __lockon__(REGISTER)
        {
            ...
            {
                XLOG|_S* _XOr("<APIF/RGS> query security info of hkey(%08x) '%s'",50,259461615)
                %args->ObjectHandle
                %o->Path();
                if ( o->IsProxy() )
                {
    ...
    virtual void LogState()
    {
        __lockon__(REGISTER)
        {
            ...
            HANDLE h;
            int i = 0;
            XLOG|_S* _XOr("<APIF/RGS> filtered: %d, virtual %d ",37,255725862)
            %REGISTER->IsFiltered() %REGISTER->IsVirtual();
            XLOG|_XOr("<APIF/RGS> --- begin of registry state --- ",44,282530127);
            while ( handles_.NextRecord(&i,&h,&r) )
                XLOG|_S* _XOr("... [%08x] %5s %s",18,276828288)
                %h
                %(r->IsProxy()?_XOr("proxy",6,279842998):_XOr("node",5,279318718))
                %r->Path();
            XLOG|_XOr("<APIF/RGS> --- persistent nodes --- ",37,280498344);
            for ( int i = 0; i < REGISTER->Stored().Count() ; ++i )
                XLOG|_S* _XOr("... %s",7,273813746) %REGISTER->Stored()[i]->Path();
            XLOG|_XOr("<APIF/RGS> --- end of registry state --- ",42,275058941);
        }
    }
