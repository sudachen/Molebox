...
void Load_Registry_Files(pwide_t mask)
{
    ...
    for (int i = 0; i < lst->Count(); ++i)
    {
        ...
        xlog | _S* _XOr("importing registry file: %s",28,341208270) % lst->At(i)->ComposePath1();
        if (0 == lst->At(i)->Read(0, +text, lst->At(i)->Size(), &wsr))
        {
            if (!Import_Registry(+text, error))
                xlog | _S* _XOr("failed to import registry file: %s",35,343174380) % error;
        }
        else
        {
            xlog | _S* _XOr("failed to read registry file: %s",33,336686129)
...
{
    xlog | _S* _XOr("ocx_register: %s",17,365784149) % fullname;
    ...
    if (HMODULE hm = LoadLibraryExW((wchar_t*)fullname, 0, LOAD_WITH_ALTERED_SEARCH_PATH))
    {
        if (!!(*(void**)&DLLregisterServer = GetProcAddress(hm, "DllRegisterServer")))
        {
            ...
            if (hr)
                xlog | _S* _XOr("  failed to register with code %d",34,366636106) % hr;
        }
    }
    SetErrorMode(olderr);
    return 1;
...
{
    xlog | _S* _XOr("commad line: %s",16,368471150) % GetCommandLineW();
    xlog | _XOr("starting at stage 0 ...",24,360869762);
...
{
    xlog | _XOr("starting at stage 1 ...",24,363163581);
    #if !defined __BUILD_VR_CORE__
...
void FixupImageName()
{
    if (int l = GetEnvironmentVariableA(_XOr("XOJUMAN-APPNAME",16,357199834), 0, 0))
    {
        //__asm int 3;
        ...
        GetEnvironmentVariableW(+StringW(_XOr("XOJUMAN-APPNAME",16,357986254)), imgpathq, l);
...
{
    if (equal1(modname, _XOr("KERNEL32.DLL",13,358707169)))
        return _KERN32;
...
void __stdcall _FirstCall(void* ntdll, void* kern32, STB_INFO* info, void* base)
{
    ...
    _USER32 = GetModuleHandleA(_XOr("user32",7,359886611));
    _ADVA32 = GetModuleHandleA(_XOr("advapi32",9,352808735));
...
void* __stdcall loadlibrary(char* dllname)
{
    ...
    if (void* m = LoadLibraryA(dllname))
        return m;
e:
    MessageBox(0, 0 | _S * _XOr("failed to load module %s",25,353660684) % dllname, _XOr("Import Error",13,356478759),
               MB_OK | MB_ICONERROR);
...
void* __stdcall getproc(void* module, char* procname)
{
    ...
    if (!proc)
    {
        ...
        MessageBox(0, 0 | _S * _XOr("module %s doesn't have symbol %s",33,382627670) % modname % procname,
                   _XOr("Import Error",13,384528251), MB_OK | MB_ICONERROR);
        TerminateProcess((HANDLE) - 1, -1);
    }
    if (module && (0
                   || module == _KERN32
...
void ReorderImports(byte_t* mapped)
{
    ...
    for (; d->Name; ++d)
    {
        ...
        if (dn.StartWith(_XOr("user32",7,385118060)) || dn.StartWith(_XOr("gdi32",6,378039960)))
            continue;
        int n = 0;
...
void ExecuteActivator(STB_INFO* info)
{
    ...
    //    *(void**)&f = GetProcAddressIndirect(base,_XOr("_Activate@4",12,377581185),0);
    //    if ( f && f(base) ) return;
    //  }
    ...
    if (base)
    {
        ...
        *(void**)&f = GetProcAddressIndirect(base, _XOr("_Activate@4",12,378760883), 0);
        if (!f) *(void**)&f = GetProcAddressIndirect(base, _XOr("Activate@4",11,379743906), 0);
        if (!f) *(void**)&f = GetProcAddressIndirect(base, _XOr("Activate",9,380726993), 0);
        ...
        {
            *(void**)&f = GetProcAddressIndirect(base, _XOr("_activate",10,373780191), 0);
            ok = (f && ((fC)f)(api));
        }
...
void ActDeactivate(ulong_t cookie)
{
    ...
    {
        HMODULE hm = GetModuleHandle(_XOr("kernel32.dll",13,374763214));
        *(void**)&fDeactivateActCtx = GetProcAddress(hm, _XOr("DeactivateActCtx",17,375418592));
    }
...
void InitActFromManifest(void* base, pwide_t mod_path, ulong_t* cookie)
{
    ...
    if (!fCreateActCtxW)
    {
        HMODULE hm = GetModuleHandle(_XOr("kernel32.dll",13,369978901));
        *(void**)&fActivateActCtx = GetProcAddress(hm, _XOr("ActivateActCtx",15,371027461));
        *(void**)&fCreateActCtxW = GetProcAddress(hm, _XOr("CreateActCtxW",14,372207159));
    }
    ...
    if (fCreateActCtxW)
    {
        ...
        if (!fA)
            xlog | _XOr("failed to activate default manifest",36,372862505);
    }
}
...
void* InitializeDotNet(STB_INFO* info, void* base)
{
    ...
    if (!fCoInitializeEE)
        if (mscoree = GetModuleHandle(_XOr("mscoree.dll",12,399732339)))
        {
            *(void**)&fCoInitializeEE = GetProcAddressIndirect(mscoree, _XOr("CoInitializeEE",15,402419300), -1);
            *(void**)&f_CorValidateImage = GetProcAddressIndirect(mscoree, _XOr("_CorValidateImage",18,395143573), -1);
            *(void**)&fCorBindToRuntime = GetProcAddressIndirect(mscoree, _XOr("CorBindToRuntime",17,395667853), -1);
            //*(void**)&fCLRCreateInstance = GetProcAddressIndirect(mscoree,_XOr("CLRCreateInstance",18,398223780),-1);
        }
    if (fCoInitializeEE)
    {
        IUnknown* pv = 0;
        ...
        void* mscorwks = GetModuleHandle(_XOr("mscorwks",9,390687193));
        void* clr = GetModuleHandle(_XOr("clr",4,391670216));

        void* snvf1 = GetProcAddressIndirect(mscoree, _XOr("StrongNameSignatureVerification",32,391539150), -1);
        void* snvf2 = GetProcAddressIndirect(mscorwks, _XOr("StrongNameSignatureVerification",32,393439507), -1);
        void* snvf4 = GetProcAddressIndirect(clr, _XOr("StrongNameSignatureVerification",32,388917558), -1);
        ...
        if (mscorwks)
            *(void**)&f_CorExeMain = GetProcAddressIndirect(mscorwks, _XOr("_CorExeMain",12,415918426), -1);
        if (!f_CorExeMain && clr)
            *(void**)&f_CorExeMain = GetProcAddressIndirect(clr, _XOr("_CorExeMain",12,416966986), -1);
        if (f_CorExeMain)
            return &ExecuteCLR;
    }
...
void ReConfigureDEP()
{
    ...
    *(void**)&fSetProcessDEPPolicy = GetProcAddressIndirect(_KERN32, _XOr("SetProcessDEPPolicy",20,417950073), -1);
...
void* Mmap_Exe_Address_Space(IMAGE_NT_HEADERS* nth, bool* relocated)
{
    ...
    {
        XLOG | _S* _XOr("<EXELOAD> relocated %08x -> %08x",33,418605203)
        % nth->OptionalHeader.ImageBase
        % p;
        *relocated = 1;
    }
    if (!p)
        XLOG | _XOr("<EXELOAD> failed to allocate address space",43,413755577);
    return p;
}
...
void* Load_Exe_From_File(pwide_t filename, bool reset_peb_base, bool kill_current)
{
    ...
    if (!fp)
    {
        XLOG | _S* _XOr("<EXELOAD> failed to open internal file '%s'",44,408512713) % filename;
    ...
    {
        XLOG | _S* _XOr("<EXELOAD> failed to read from file '%s\\%s'",43,403400731)
        % fp->PackageName() % fp->ComposePath1();
        __asm mov ecx, __LINE__;
        __asm int 3;
    }
    ...
    if (kill_current)
    {
        ...
        {
            XLOG | _S* _XOr("<EXELOAD> failed to unmap original PE",38,406480938);
            __asm mov ecx, __LINE__;
            __asm int 3;
        }
    }
    ...
    if (0 != fp->Read(0, base, 4 * 1024, &wasread))
    {
        XLOG | _S* _XOr("<EXELOAD> failed to read from file '%s\\%s'",43,433285235)
        % fp->PackageName() % fp->ComposePath1();
        __asm mov ecx, __LINE__;
        __asm int 3;
    }
    ...
    for (int i = 0; i < nth->FileHeader.NumberOfSections; ++i)
    {
        ...
        if (S->PointerToRawData && S->SizeOfRawData)
            if (0 != fp->Read(
                    ...
                    &wasread))
            {
                XLOG | _S* _XOr("<EXELOAD> failed to read section %d from file '%s\\%s'",54,427846528)
                % i % fp->PackageName() % fp->ComposePath1();
                __asm mov ecx, __LINE__;
                __asm int 3;
            }
        ...
        XOJUMAN_STARTUP_INFO* sti)
{
    ...
    _USER32 = GetModuleHandleA(_XOr("user32",7,424176600));
    _ADVA32 = GetModuleHandleA(_XOr("advapi32",9,423783362));
    _imp__GetModuleHandleW__4 = _GetModuleHandleW;
    _imp__GetProcAddress__8 = _GetProcAddress;
    ...
    XLOG | _XOr("working in Injectin mode",25,425028557);
        ...
        void* key, void* core, int core_size)
{
    ...
    g_License_Level = 100;
    _Inity(flags, 0);
    XLOG | _XOr("working in SDK mode",20,427322344);
    g_Core_Handle = core;
    g_Inject_Flags = flags;
    g_Inject_Into_Child = flags & TEGGOVBOX_CORE_INJECT;
