...
void Load_Registry_Files(pwide_t mask)
{
    ...
    for (int i = 0; i < lst->Count(); ++i)
    {
        ...
        xlog | _S* _XOr("importing registry file: %s",28,367137830) % lst->At(i)->ComposePath1();
        if (0 == lst->At(i)->Read(0, +text, lst->At(i)->Size(), &wsr))
        {
            if (!Import_Registry(+text, error))
                xlog | _S* _XOr("failed to import registry file: %s",35,335549440) % error;
        }
        else
        {
            xlog | _S* _XOr("failed to read registry file: %s",33,339220440)
...
{
    xlog | _S* _XOr("ocx_register: %s",17,340924342) % fullname;
    ...
    if (HMODULE hm = LoadLibraryExW((wchar_t*)fullname, 0, LOAD_WITH_ALTERED_SEARCH_PATH))
    {
        if (!!(*(void**)&DLLregisterServer = GetProcAddress(hm, "DllRegisterServer")))
        {
            ...
            if (hr)
                xlog | _S* _XOr("  failed to register with code %d",34,341907363) % hr;
        }
    }
    SetErrorMode(olderr);
    return 1;
...
{
    xlog | _S* _XOr("commad line: %s",16,345708409) % GetCommandLineW();
    xlog | _XOr("starting at stage 0 ...",24,346494821);
...
{
    xlog | _XOr("starting at stage 1 ...",24,348723019);
    #if !defined __BUILD_VR_CORE__
...
void FixupImageName()
{
    if (int l = GetEnvironmentVariableA(_XOr("XOJUMAN-APPNAME",16,351082287), 0, 0))
    {
        //__asm int 3;
        ...
        GetEnvironmentVariableW(+StringW(_XOr("XOJUMAN-APPNAME",16,351999769)), imgpathq, l);
...
{
    if (equal1(modname, _XOr("KERNEL32.DLL",13,386144004)))
        return _KERN32;
...
void __stdcall _FirstCall(void* ntdll, void* kern32, STB_INFO* info, void* base)
{
    ...
    _USER32 = GetModuleHandleA(_XOr("user32",7,387061494));
    _ADVA32 = GetModuleHandleA(_XOr("advapi32",9,388568811));
...
void* __stdcall loadlibrary(char* dllname)
{
    ...
    if (void* m = LoadLibraryA(dllname))
        return m;
e:
    MessageBox(0, 0 | _S * _XOr("failed to load module %s",25,389879519) % dllname, _XOr("Import Error",13,390141635),
               MB_OK | MB_ICONERROR);
...
void* __stdcall getproc(void* module, char* procname)
{
    ...
    if (!proc)
    {
        ...
        MessageBox(0, 0 | _S * _XOr("module %s doesn't have symbol %s",33,393156269) % modname % procname,
                   _XOr("Import Error",13,394991241), MB_OK | MB_ICONERROR);
        TerminateProcess((HANDLE) - 1, -1);
    }
    if (module && (0
                   || module == _KERN32
...
void ReorderImports(byte_t* mapped)
{
    ...
    for (; d->Name; ++d)
    {
        ...
        if (dn.StartWith(_XOr("user32",7,395974270)) || dn.StartWith(_XOr("gdi32",6,395384435)))
            continue;
        int n = 0;
...
void ExecuteActivator(STB_INFO* info)
{
    ...
    //    *(void**)&f = GetProcAddressIndirect(base,_XOr("_Activate@4",12,396760682),0);
    //    if ( f && f(base) ) return;
    //  }
    ...
    if (base)
    {
        ...
        *(void**)&f = GetProcAddressIndirect(base, _XOr("_Activate@4",12,397678164), 0);
        if (!f) *(void**)&f = GetProcAddressIndirect(base, _XOr("Activate@4",11,398857802), 0);
        if (!f) *(void**)&f = GetProcAddressIndirect(base, _XOr("Activate",9,400037432), 0);
        ...
        {
            *(void**)&f = GetProcAddressIndirect(base, _XOr("_activate",10,401217070), 0);
            ok = (f && ((fC)f)(api));
        }
...
void ActDeactivate(ulong_t cookie)
{
    ...
    {
        HMODULE hm = GetModuleHandle(_XOr("kernel32.dll",13,402527778));
        *(void**)&fDeactivateActCtx = GetProcAddress(hm, _XOr("DeactivateActCtx",17,369890828));
    }
...
void InitActFromManifest(void* base, pwide_t mod_path, ulong_t* cookie)
{
    ...
    if (!fCreateActCtxW)
    {
        HMODULE hm = GetModuleHandle(_XOr("kernel32.dll",13,370678264));
        *(void**)&fActivateActCtx = GetProcAddress(hm, _XOr("ActivateActCtx",15,371595754));
        *(void**)&fCreateActCtxW = GetProcAddress(hm, _XOr("CreateActCtxW",14,372578775));
    }
    ...
    if (fCreateActCtxW)
    {
        ...
        if (!fA)
            xlog | _XOr("failed to activate default manifest",36,373496257);
    }
}
...
void* InitializeDotNet(STB_INFO* info, void* base)
{
    ...
    if (!fCoInitializeEE)
        if (mscoree = GetModuleHandle(_XOr("mscoree.dll",12,377100702)))
        {
            *(void**)&fCoInitializeEE = GetProcAddressIndirect(mscoree, _XOr("CoInitializeEE",15,378018184), -1);
            *(void**)&f_CorValidateImage = GetProcAddressIndirect(mscoree, _XOr("_CorValidateImage",18,378935674), -1);
            *(void**)&fCorBindToRuntime = GetProcAddressIndirect(mscoree, _XOr("CorBindToRuntime",17,379787617), -1);
            //*(void**)&fCLRCreateInstance = GetProcAddressIndirect(mscoree,_XOr("CLRCreateInstance",18,382605650),-1);
        }
    if (fCoInitializeEE)
    {
        IUnknown* pv = 0;
        ...
        void* mscorwks = GetModuleHandle(_XOr("mscorwks",9,383457593));
        void* clr = GetModuleHandle(_XOr("clr",4,384571692));

        void* snvf1 = GetProcAddressIndirect(mscoree, _XOr("StrongNameSignatureVerification",32,384244005), -1);
        void* snvf2 = GetProcAddressIndirect(mscorwks, _XOr("StrongNameSignatureVerification",32,419502339), -1);
        void* snvf4 = GetProcAddressIndirect(clr, _XOr("StrongNameSignatureVerification",32,423434463), -1);
        ...
        if (mscorwks)
            *(void**)&f_CorExeMain = GetProcAddressIndirect(mscorwks, _XOr("_CorExeMain",12,425400505), -1);
        if (!f_CorExeMain && clr)
            *(void**)&f_CorExeMain = GetProcAddressIndirect(clr, _XOr("_CorExeMain",12,426580143), -1);
        if (f_CorExeMain)
            return &ExecuteCLR;
    }
...
void ReConfigureDEP()
{
    ...
    *(void**)&fSetProcessDEPPolicy = GetProcAddressIndirect(_KERN32, _XOr("SetProcessDEPPolicy",20,427759773), -1);
...
void* Mmap_Exe_Address_Space(IMAGE_NT_HEADERS* nth, bool* relocated)
{
    ...
    {
        XLOG | _S* _XOr("<EXELOAD> relocated %08x -> %08x",33,428152967)
        % nth->OptionalHeader.ImageBase
        % p;
        *relocated = 1;
    }
    if (!p)
        XLOG | _XOr("<EXELOAD> failed to allocate address space",43,431888482);
    return p;
}
...
void* Load_Exe_From_File(pwide_t filename, bool reset_peb_base, bool kill_current)
{
    ...
    if (!fp)
    {
        XLOG | _S* _XOr("<EXELOAD> failed to open internal file '%s'",44,433068080) % filename;
    ...
    {
        XLOG | _S* _XOr("<EXELOAD> failed to read from file '%s\\%s'",43,402855937)
        % fp->PackageName() % fp->ComposePath1();
        __asm mov ecx, __LINE__;
        __asm int 3;
    }
    ...
    if (kill_current)
    {
        ...
        {
            XLOG | _S* _XOr("<EXELOAD> failed to unmap original PE",38,405806032);
            __asm mov ecx, __LINE__;
            __asm int 3;
        }
    }
    ...
    if (0 != fp->Read(0, base, 4 * 1024, &wasread))
    {
        XLOG | _S* _XOr("<EXELOAD> failed to read from file '%s\\%s'",43,409476008)
        % fp->PackageName() % fp->ComposePath1();
        __asm mov ecx, __LINE__;
        __asm int 3;
    }
    ...
    for (int i = 0; i < nth->FileHeader.NumberOfSections; ++i)
    {
        ...
        if (S->PointerToRawData && S->SizeOfRawData)
            if (0 != fp->Read(
                    ...
                    &wasread))
            {
                XLOG | _S* _XOr("<EXELOAD> failed to read section %d from file '%s\\%s'",54,412818297)
                % i % fp->PackageName() % fp->ComposePath1();
                __asm mov ecx, __LINE__;
                __asm int 3;
            }
        ...
        XOJUMAN_STARTUP_INFO* sti)
{
    ...
    _USER32 = GetModuleHandleA(_XOr("user32",7,415439681));
    _ADVA32 = GetModuleHandleA(_XOr("advapi32",9,416881467));
    _imp__GetModuleHandleW__4 = _GetModuleHandleW;
    _imp__GetProcAddress__8 = _GetProcAddress;
    ...
    XLOG | _XOr("working in Injectin mode",25,418061097);
        ...
        void* key, void* core, int core_size)
{
    ...
    g_License_Level = 100;
    _Inity(flags, 0);
    XLOG | _XOr("working in SDK mode",20,453778188);
    g_Core_Handle = core;
    g_Inject_Flags = flags;
    g_Inject_Into_Child = flags & TEGGOVBOX_CORE_INJECT;
