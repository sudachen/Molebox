...
struct ActCtxFilter : APIfilter
{
        ...
        rtlCreateActivationContext(0)
    {
        *(void**)&rtlCreateActivationContext = GetProcAddressIndirect(_NTDLL,
		_XOr("RtlCreateActivationContext",27,27853289),0);
        if ( rtlCreateActivationContext )
        {
            HMODULE hm = LoadLibrary(_XOr("sxs.dll",8,21955023));
            if ( hm )
                *(void**)&sxsGenerateActivationContext = GetProcAddressIndirect(hm,
		_XOr("SxsGenerateActivationContext",29,24969533),0);
        }
    ...
    )
    {
        ...
        if ( sxsGenerateActivationContext(&sxsd) )
            if ( sxsd.Mapping(osversion_) )
            {
                ...
                {
                    XLOG | _S* _XOr("<APIF/CTX> RtlCrateActivationContext failed with error %08x",60,18809119)%r;
                    UnmapViewOfFile(actctx);
                }
                else
                {
                    XLOG | _S* _XOr("<APIF/CTX> activation context successful created -> %08x",57,48103774)%*result;
                    return true;
                }
            }

        XLOG | _S* _XOr("<APIF/CTX> SxsGenerateActivationContext failed",47,43126418);
        return false;
    }
    ...
    APIF_STATUS DoCreateActCtxW(void* _a, unsigned* result)
    {
        ...
        XDBG|_S* _XOr(".apif.CreatActCtxW ptr: %08x",29,44240611) % args->ptr;
        XDBG|_S* _XOr(".apif.CreatActCtxW flags: %08x, proc: %08x, resid: %08x, langid: %08x, hmod: %08x",82,38538956)
        % args->ptr->dwFlags
        ...
        if ( (void*)sxsGenerateActivationContext != (void*)-1 )
        {
            ...
            if (manifest || config)
            {
                ...
                XLOG | _S* _XOr("<APIF/CTX> failed to found manifest for %s/%d/%d",49,65474471)
