...
namespace teggo
{
    ...
    struct DataSource_
    {
        ...
        _TEGGO_EXPORTABLE static void* CXX_STDCALL Mmap(StringParam identifier, u32_t size = 0, u32_t offset = 0, bool writable = false,
    ...
    _TEGGO_EXPORTABLE rcc_ptr<DataStream> CXX_STDCALL CreateDataStreamFromHANDLE( HANDLE fd, StringParam fname, int dupit,
            int opt );

    enum { RTRY_COUNT = 10, RTRY_TIMEOUT = 300 };

    inline rcc_ptr<DataStream> CXX_STDCALL OpenStreamRtry(StringParam fname, unsigned flags, int count = RTRY_COUNT,
            int timeout = RTRY_TIMEOUT )
    {
    ...
    inline rcc_ptr<DataStream> CXX_STDCALL OpenHandleStreamRtry(StringParam fname , unsigned flags, int count = RTRY_COUNT,
            int timeout = RTRY_TIMEOUT )
    {
        for ( int i = 0; i < count; ++i )
        {
