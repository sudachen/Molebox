...
namespace teggo {
  ...
  _TEGGO_EXPORTABLE void *CXX_STDCALL SysMmapFile(StringParam fname, u32_t size = 0, u32_t offset = 0, MMAP_TYPE tp=MMAP_RW, EhFilter ehf = PRINTEF);
  _TEGGO_EXPORTABLE void *CXX_STDCALL SysMmapFile(StringParam fname, u32_t size = 0, u32_t offset = 0, bool writable = true, bool create_new = false, EhFilter ehf = PRINTEF);
  _TEGGO_EXPORTABLE void *CXX_STDCALL SysMmapFileByHandle(HANDLE handle, u32_t size = 0, u32_t offset = 0, bool writable = true, EhFilter ehf = PRINTEF);
  _TEGGO_EXPORTABLE void  CXX_STDCALL SysUnmmapFile(void const*);
  _TEGGO_EXPORTABLE StringT<char> CXX_STDCALL OemRecode(pchar_t);
  _TEGGO_EXPORTABLE bool CXX_STDCALL SysPipe(pwide_t command, FILE **in, FILE **out, int *proc, bool textmode, bool useCmd, EhFilter ehf = PRINTEF);
...
extern "C" HRESULT CXX_STDCALL SHGetFolderPathW(HWND hwndOwner,int nFolder,HANDLE hToken,DWORD dwFlags,LPWSTR pszPath);
  ...
  _TEGGO_SYSUTIL_FAKE_INLINE
  void *CXX_STDCALL SysMmapFile2(StringParam fname, u32_t size, u32_t offset, bool writable, bool create_new, bool cow, EhFilter ehf)
    {
