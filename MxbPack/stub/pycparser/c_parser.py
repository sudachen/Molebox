...
class CParser(PLYParser):    
    ...
    def p_external_declaration_4(self,p):
        """ external_declaration    : QUOTE compound_string """
        p[0] = [c_ast.Quote(p[2],self._coord(p.lineno(1)))]
    ...
    # line is reached.
    #
    def p_declaration_a(self, p):
        """ declaration_a : decl_body SEMI 
        """
        p[0] = p[1]

    def p_declaration_b(self, p):
        """ declaration_b : declspec decl_body SEMI 
        """
        p[0] = p[2]
        for i in p[0]:
            i.type.attributes += [p[1]]

    def p_declaration_c(self, p):
    ...
    def p_attribute_expr(self,p):
        ...
        """
        if len(p) == 5:
            p[0] = c_ast.Attribute(name=p[1],expr=p[3],coord=self._coord(p.lineno(1)))
        else:
            p[0] = c_ast.Attribute(name=p[1],coord=self._coord(p.lineno(1)))
                                  
    def p_attribute(self,p):
        """ attribute : ATTRIBUTE LPAREN LPAREN attribute_expr RPAREN RPAREN """
        p[0] = p[4]
    
    def p_declspec(self,p):
        """ declspec : DECLSPEC LPAREN ID RPAREN """
        p[0] = c_ast.Declspec(name=p[3],expr=None,coord=self._coord(p.lineno(1)))

    def p_direct_declarator_1(self, p):
        """ direct_declarator   : STDCALL direct_declarator
        """
        p[0] = p[2]
    ...
    def p_compound_string(self,p):
        """ compound_string : compound_string STRING_LITERAL
                            | STRING_LITERAL
        """
        if len(p) == 3:
            p[0] = '"'+(p[1].strip('"') + p[2].strip('"'))+'"'
            #sys.stderr.write("{"+repr(p[0])+"}")
            #sys.stderr.write("\n")
        else:
            p[0] = p[1]
            
    def p_asm_argument(self,p):
        """ asm_argument : compound_string LPAREN expression RPAREN """
        p[0] = (p[1],p[3])
        
    def p_asm_arguments_list(self,p):
        """ asm_arguments_list : asm_argument COMMA asm_arguments_list 
                               | asm_argument
        """ 
        if len(p) == 4:
            p[0] = [p[1]] + p[3]
    ...
    def p_asm_param_list(self,p):
        ...
        if len(p) == 4:
            #p[0] = [p[2]] + [p[3]]
            p[0] = [p[2]]
            if p[3]: p[0].extend(p[3])
        elif len(p) == 3:
            p[0] = [None,] + p[2]
        else:
            p[0] = None
                        
    def p_asm_qulifier(self,p):
    ...
    def p_asm_expression(self,p):
        """ asm_expression : LPAREN asm_expression RPAREN
                           | LPAREN compound_string asm_param_list RPAREN
        """                  
        if len(p) == 5:
            p[0] = { 'code': p[2], 'params': p[3] }
        else:
            p[0] = p[2]                  
                                          
    def p_asm_statement(self,p):
        """ asm_statement : asm_qualifier asm_expression SEMI """
        p[0] = c_ast.Assembler(p[1],p[2]['code'],p[2]['params'],self._coord(p.lineno(1)))
    
    def p_quote_statement(self,p):
        """ quote_statement : QUOTE compound_string SEMI """
        p[0] = c_ast.Quote(p[2],self._coord(p.lineno(1)))
