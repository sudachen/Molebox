...
class FuncDecl(Node):
    ...
    def children(self):
        ...
        if self.args is not None: nodelist.append(self.args)
        if self.type is not None: nodelist.append(self.type)
        if self.attributes: nodelist.extend(self.attributes)
        return tuple(nodelist)
...
class TypeDecl(Node):
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        ...
        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in \
                [("declname", repr(self.declname)),\
                 ("quals", repr(self.quals)),
...
class Assembler(Node):
    def __init__(self, volatile, code, params, coord=None):
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'Assembler: ')

        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in [("volatile", repr(self.volatile)), ("code", repr(self.code)), ("params", repr(self.params))])
        else:
            attrstr = ', '.join('%s' % v for v in [self.volatile,self.code,self.params])
        buf.write(attrstr)
...
class Attribute(Node):
    def __init__(self, name, expr=None, coord=None):
        self.name = name
        self.expr = expr
        self.coord = coord
    ...
    def children(self):
        nodelist = []     
        if self.expr: nodelist.append(self.expr) 
        return tuple(nodelist)

    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'Attribute: ')
...
class Quote(Node):
    ...
    def show(self, buf=sys.stdout, offset=0, attrnames=False, showcoord=False):
        lead = ' ' * offset
        buf.write(lead + 'Quote: ')

        if attrnames:
            attrstr = ', '.join('%s=%s' % nv for nv in [("code", repr(self.code))])
        else:
            attrstr = ', '.join('%s' % v for v in [self.code])
        buf.write(attrstr)
