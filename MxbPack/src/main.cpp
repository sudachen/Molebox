...
{
	EhFilter ehf = EhOut();
	cargs = ParseCargs(argc,argv,"V?hHno:m:P:p:r:Rde:a:BqQlLTOUg:G:D:c:C:012FfS:s89tk:","verbose,help,version,nologo,usereg,inject,regfull,ncr,company:,regmask:,image:,tls:,autoreg:,sdk:",ehf);
	...
	if (cargs && cargs->args.Count() && cargs->HasNoOpts("V?hrR","help,version"))
	{
		...
		if ( cargs->args[0] != L"~~" ) inputfile = cargs->args[0];
			...
			if ( !process_executable ) inputfile = "";
