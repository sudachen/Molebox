...
bool InitTlsDataSection(byte_t *mapped,IMAGE_SECTION_HEADER *S,IMAGE_NT_HEADERS *nth,unsigned *file_length,unsigned reserved, unsigned inited, EhFilter ehf);
bool InitRSRCSection(byte_t *mapped,IMAGE_SECTION_HEADER *S,IMAGE_NT_HEADERS *nth,unsigned *file_length,BufferT<byte_t> &rsc,EhFilter ehf);
...
FileContainerPtr OpenStubbedFileContainer(StringParam original,StringParam target, StringParam stub, unsigned flags, unsigned minSize, unsigned minTls, EhFilter ehf);
...
bool AfxBundleDLL(StringParam dllname, StringA &module, StringParam target, BUNDLED_IMPORT &bi, byte_t *xcrypt_key, EhFilter ehf);
