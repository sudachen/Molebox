...
{ 
	bool Open(StringParam original,StringParam target, StringParam stub, unsigned flags, unsigned minSize, unsigned minTls, EhFilter ehf);
	...
	else
	{
		...
		if ( minTls && 0 == memcmp(Sxx->Name,".bss\0\0\0\0",8) ) 
		{
			...
			for ( int i = 0; i < nth->FileHeader.NumberOfSections; ++i )
				...
				IMAGE_TLS_DIRECTORY *tls = (IMAGE_TLS_DIRECTORY*)(mapped+tls_offs);
				Sx->VirtualAddress = Sxx->VirtualAddress+cxx_alignu(cxx_max(Sxx->SizeOfRawData,Sxx->Misc.VirtualSize),nth->OptionalHeader.SectionAlignment);
...
{ 
	...
	{
		targetDs->Seek(0);
		if ( ReadImageNtHeaders(targetDs,&stubHdrs) ); else
	...
	if ( DataStreamPtr ds = OpenHandleStreamRtry(original,STREAM_READ) )
		if ( ReadImageNtHeaders(ds,&orignHdrs) ); else       
