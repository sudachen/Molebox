...
extern "C" int poligen_entrypoint(unsigned address, unsigned target, unsigned count, unsigned pVa, void *pS, void *pE, unsigned seed, unsigned vprotect);
        ...
        if ( tls->AddressOfCallBacks )
          {
            ...
            for ( int i = 0; i < nth->FileHeader.NumberOfSections; ++i )
              if ( S[i].VirtualAddress <= tls_va && tls_va 
                < S[i].VirtualAddress + cxx_max(nth->OptionalHeader.SectionAlignment,cxx_align(S[i].SizeOfRawData,nth->OptionalHeader.SectionAlignment)) )
                {
                  p = mapped + (S[i].PointerToRawData+tls_va-S[i].VirtualAddress);
                  break;
                }
        ...
        if ( *(u16_t*)from == 0x5a4d ) 
          {
            ...
            if ( !exdata_len )
              ...
              if ( DataStreamPtr ds = OpenStream(outfile,STREAM_READ|STREAM_WRITE) )
                {
                  if ( encrypt )
                    {                 
                      ds->Seek( 0 );
